...
//total
       // dd($request->totales);
        ...
        foreach (Cart::content() as $row) {
            ...
            $total_items = $total_items + $row->qty;
            $pedido->productos()->attach($producto, ['cantidad' => $row->qty, 'pedido_id' => $pedidoid, 'medida' => $row->options->medida, 'codigo' => $row->options->codigo, 'producto_id' => $row->id, 'costo' => $row->price * $row->qty, 'iva' => $total_ivap, 'total' => $total_costo]);
      ...
      /*  Mail::send('privada.mailpedido', ['total' => $totales, 'username' => $username, 'nombre' => $nombre, 'apellido' => $apellido, 'social' => $social, 'cuit' => $cuit, 'telefono' => $telefono, 'direccion' => $direccion, 'emailcliente' => $emailcliente, 'items' => $items, 'row' => $row, 'subtotal' => $subtotal, 'mensaje' => $mensaje, 'iva' => $total_iva, 'descuento' => $descuento], function ($message) use ($nombre, $apellido) {
            ...
            $message->to('juani.barroso@hotmail.com');

            //Add a subject
            $message->subject('Pedido de ' . $nombre . ' ' . $apellido);

        });*/
        Mail::send('privada.mailpedido', ['total' => $totales, 'username' => $username, 'nombre' => $nombre, 'apellido' => $apellido, 'social' => $social, 'cuit' => $cuit, 'telefono' => $telefono, 'direccion' => $direccion, 'emailcliente' => $emailcliente, 'items' => $items, 'row' => $row, 'subtotal' => $subtotal, 'mensaje' => $mensaje, 'iva' => $total_iva, 'descuento' => $descuento], function ($message) use ($nombre, $apellido, $emailcliente) {
            ...
            //Add a subject
            $message->subject('Su pedido ' . $nombre . ' ' . $apellido);
